...
const ReceivedProcessing: React.FC = () => {
  ...
  const getReceived = async (): Promise<void> => {
    ...
    try {
      ...
      if (response.ok && Array.isArray(result)) {
        ...
        console.error('Failed to fetch received data or user not selected');
      }
    } catch (error) {
  ...
  return (
    <SafeAreaView style={styles.searchcontainer}>
      <TextInput
        ...
        // You can implement filtering functionality if needed here
      />
...
export default ReceivedProcessing;
...
const styles = StyleSheet.create({
  ...
  buttonContainer: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    gap: 10, // works on React Native 0.71+; if unsupported, you can use marginHorizontal on buttons
  },
  button: {
    flex: 1,
    padding: 12,
